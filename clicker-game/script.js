...
const DREAMLO_URL = 'https://www.dreamlo.com/lb/' + DREAMLO_CODE;
...
function loadGame() {
    ...
    if (json) {
        try {
            ...
            updateUI();
            // alert("セーブデータをロードしました！"); // Alert is too intrusive on load
...
function submitOnlineScore(level) {
    ...
    // Dreamlo Add: /add/{name}/{score}
    const url = `${DREAMLO_URL}/add/${encodeURIComponent(name)}/${level}`;
